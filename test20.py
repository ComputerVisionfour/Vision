...
img2 = clahe.apply(img_stratched2)

kernel1 = cv2.getStructuringElement(cv2.MORPH_RECT, (8,8))
blackhat = cv2.morphologyEx(img2, cv2.MORPH_BLACKHAT, kernel1)

img4 = cv2.normalize(blackhat, None, 0, 255, cv2.NORM_MINMAX) # 히스토그램 스트레칭은 NORM_MINMAX

img5 = cv2.normalize(img4, None, 0, 255, cv2.NORM_MINMAX) # 히스토그램 스트레칭은 NORM_MINMAX

img6 = cv2.normalize(img5, None, 0, 255, cv2.NORM_MINMAX) # 히스토그램 스트레칭은 NORM_MINMAX

img7 = cv2.normalize(img6, None, 0, 255, cv2.NORM_MINMAX) # 히스토그램 스트레칭은 NORM_MINMAX
...
try:
    ...
    for i in circles[0]:
        ...
        cv2.rectangle(img_c,(x-50,y-25),(x+50,y+25),(255,255,255),2)
        img_crop = img7[y-25:y+25,x-50:x+50] # 이미지 크롭
